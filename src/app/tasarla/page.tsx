...
export default function DesignPage() {
    const [engravingText, setEngravingText] = useState("SULEYMAN");
        ...
        <main className="min-h-screen pt-32 pb-24 px-6 bg-bg-primary flex flex-col lg:flex-row h-screen">
            ...
            <div className="w-full lg:w-2/3 h-[50vh] lg:h-auto relative bg-bg-secondary/20 border border-gold/30 order-1 lg:order-2">
                ...
                <Canvas shadows camera={{ position: [0, 0, 6], fov: 45 }}>
                    ...
                    <OrbitControls minPolarAngle={Math.PI / 4} maxPolarAngle={Math.PI / 1.5} enableZoom={false} />
                </Canvas>
            </div>
                ...
                <div className="space-y-8">
                    <div>
                        <label className="block text-xs font-sans uppercase tracking-[0.2em] text-gray-400 mb-3">
                            Yazılacak Metin (Max 10 Karakter)
                        </label>
                        <input
                            type="text"
                            maxLength={10}
                            ...
                            className="w-full border-b border-bg-secondary py-2 font-serif text-2xl text-text focus:outline-none focus:border-gold bg-transparent transition-colors placeholder:text-gray-300 uppercase"
                            placeholder="ISIM"
                        />
                    </div>
